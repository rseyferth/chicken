...
export default class JsonApi extends Api
{
	...
	call(method : string, uri : string, data? : any, ajaxOptions? : JQueryAjaxSettings) {
	...
	serialize(model : any, includeRelated : boolean =  true, includeRelatedData : any = false, includedModelGuids : string[] = []) {
		...
		if (model.isPivot()) {
			...
			if (_.size(pivotAttributes) > 0) {
				...
				// Convert for API
				let meta = {};
				_.each(pivotAttributes, (value, key : string) => {
				...
				_.each(model.related, (relatedData, key) => {
					...
					if (relatedData instanceof Collection) {
						...
						if (relatedData.isDirty() || relatedData.hasDirtyChildren()) {
							...
							relationships[key] = { data: _.map(relatedData.items, (item) => {
								...
								if (includeRelatedData === false || !_.contains(includeRelatedData, key)) {
									includedModelGuids.push(uidFor(item));
								}
						...
						if (includeRelatedData === false || _.indexOf(includeRelatedData, key) === -1) {
							includedModelGuids.push(uidFor(relatedData));
						}
	...
	deserialize(result, apiCall? : ApiCall) {
	...
	deserializeModel(data, apiCall, deserializeRelationships = true) {
		...
		// Collect attributes
		let attributes : Dictionary<any> = {};
		_.each(data.attributes, (value, key : string) => {
		...
		// Meta?
		if (data.meta) {
			let metaAttributes : Dictionary<any> = {};
			_.each(data.meta, (value, key : string) => {
	...
	deserializeCollection(data : Dictionary<any>, apiCall : ApiCall, meta? : MetaData) {
		...
		_.each(data, (recordData) => {
			collection.addFromApi(this.deserializeModel(recordData, apiCall));
		});
	...
	_deserializeRelationships(data : Dictionary<any>, apiCall : ApiCall, model? : Model) {
		...
		if (data.relationships) {
			...
			_.each(data.relationships, (rel : Dictionary<any>, relationshipName : string) => {
	...
	_getModelName(resourceType : string) {
