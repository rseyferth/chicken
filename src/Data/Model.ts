...
import { uidFor, areEqual, getValue } from '../Helpers/Utils';
...
export default class Model extends Observable
{
	...
	private relationshipStudies : Dictionary<any> = {};

	private _scheduleUpdateDirtyTimeout : any;
	 ...
	 */
	constructor(initValues : Dictionary<any> = {}, convertToObservables : boolean = false) {
	...
	_get(key : string) : any {
		...
		// Is there a getter?
		let methodName : string = 'get' + inflection.camelize(key);
	 ...
	 */
	observe(keyOrKeys : string | string[], callback) {
	 ...
	 * @return {string|null}
	 */
	translation(key, language? : string) {

		// Language given?
		if (!language) language = Application.getInstance().i18n.language;
	...
	setAttributesFromApi(attributes) {
		...
		_.each(attributes, (value, key : string) => {
	...
	getApi() : Api {
		...
		// Check model definition
		let apiName : string = this.getDefinition() ? this.getDefinition()!.api : 'default';
		return Application.getInstance().apis[apiName];
	...
	save(options? : ModelSaveOptions) : Promise<any> {
		...
		// Make settings
		let settings : ModelSaveOptions = $.extend({
			uri: null,
			includeRelated: true,
	...
	delete(options? : ModelDeleteOptions) {
		...
		// Make settings
		let settings : ModelDeleteOptions = $.extend({
			uri: null,
	...
	toObject(includedUids : string[] = []) : Dictionary<any> {
		...
		_.each(this.related, (item, key) => {
			...
			// Observable?
			if ((<Observable>item).toObject) {

				item = item.toObject(includedUids);

			} else if ((<ObservableArray>item).toArray) {
	...
	resetDirty(keys? : any) {
		...
		// Null?
		if (!keys) keys = _.keys(this.attributes);
		if (typeof keys === 'string') keys = [keys];

		// Specific key?
		_.each(<string[]>keys, (key) => {
	 ...
	 * @chainable
	 */
	addRelatedModel(relationshipName : string, relatedModel : Model, fromApi : boolean = false, pivotAttributes? : Dictionary<any>) {
		...
		if (this.related[relationshipName] === undefined) {
			...
			if (relationship) {
				...
				this.related[relationshipName] = new Collection(<ModelConstructor>relatedModel.constructor);
			}
		...
		if (relationship && relationship.isPivot() && relatedModel.isPivot) {
			...
			relatedModel = createPivotFor(relatedModel, pivotAttributes);
	...
	deleteRelatedModel(relationshipName, model) {
		...
		// Check if collection exists
		let relationship : Relationship = this.getRelationship(relationshipName)!;
	...
	getDefinition() : ModelDefinition | undefined {
		return (<any>this.constructor).definition;
	}
	...
	createCopy() {
		...
		// Create model
		let constr : any = this.constructor;
     ...
     * @type {Map}
     */
    static registry : Map<string, ModelConstructor> = new Map();
     ...
     * @type {Map}
     */
    static stores : Map<string, ModelStore> = new Map();
    ...
    static getFromStore(modelName : string, id : any) : Model | undefined {
        ...
        // Is there a store?
        if (!Model.stores.has(modelName)) return undefined;
    ...
    static deleteFromStore(modelName : string, id : string) : void {
        ...
        //Is there a store
        if (!Model.stores.has(modelName)) return;
     ...
     * @type {Number}
     */
    static UpdateDirtyDelay : number = 100;
